 ...
 *     - {Array} ignoreQueryParams: An Array of query params key name in url.
 *   }
 * @param options
...
exports = module.exports = function webcache(cache, rules, options) {
  ...
  var defaultIgnoreQueryParams = options.ignoreQueryParams || [];
  ...
  for (var i = 0; i < rules.length; i++) {
    ...
    var ignoreQueryParams = rule.ignoreQueryParams || defaultIgnoreQueryParams;
    rule.ignoreQueryParamsRE = [];
    // like '?spm=2.3.45&' <=> /(\&|\?)spm[^\&]*\&?/
    for (var j = 0; j < ignoreQueryParams.length; j ++) {
      rule.ignoreQueryParamsRE[j] = new RegExp('(\\&|\\?)' + ignoreQueryParams[j] + '[^\\&]*\\&?' , 'ig');
    }
  ...
  return function webcache(req, res, next) {
    ...
    for (var i = 0; i < matchRule.ignoreQueryParamsRE.length; i ++) {
      url = url.replace(matchRule.ignoreQueryParamsRE[i], function (match0, match1) { 
        // if first char is '?', leave it
        if (match1 === '?') { 
          return match1; 
        }
        return ''; 

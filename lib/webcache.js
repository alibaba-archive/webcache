...
exports = module.exports = function webcache(cache, rules, options) {
  ...
  for (var i = 0; i < rules.length; i++) {
    ...
    rule.ignoreQueryParams = rule.ignoreQueryParams || defaultIgnoreQueryParams;
  ...
  return function webcache(req, res, next) {
    ...
    for (var i = 0; i < matchRule.ignoreQueryParams.length; i ++) {
      // 'www.baidu.com/?param1=234&xx=xxx'
      var params = new RegExp('(\\&|\\?)' + matchRule.ignoreQueryParams[i] + '[^\\&]*\\&?' , 'ig');
      url = url.replace(params, function (match0, match1) { 
